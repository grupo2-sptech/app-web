...
create table processador (
    id_processador int primary key auto_increment,
    nome_processador varchar (100),
    fabricante_processador varchar (100),
    frequencia_processador varchar (40),
    uso_processador double,
	hora_data_processador datetime default current_timestamp
) auto_increment = 400;

-- Tabela: memoria_ram
create table memoria_ram (
    id_memoria_ram int primary key auto_increment,
    total_ram_gb long,
    uso_ram_gb long,
	hora_data_ram datetime default current_timestamp
) auto_increment = 500;
...
create table disco (
    ...
    modelo varchar(100),
    tamanho_total long,
    tamanho_disponivel long
) auto_increment = 600;

-- Tabela: maquina
create table maquina (
    maquina_id varchar(50) primary key,
...
alter table maquina add constraint foreign key(fk_processador) references processador(id_processador);
alter table maquina add constraint foreign key(fk_ram) references memoria_ram(id_memoria_ram);
alter table maquina add constraint foreign key(fk_disco) references disco(id_disco);

-- Tabela: uso_maquina
create table uso_maquina (
    fk_funcionario int,
    fk_maquina varchar(50),
